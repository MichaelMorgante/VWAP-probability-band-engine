...
def plot_probability_heatmap(prob_table: pd.DataFrame,
    ...
    # ── Context overlay: execution + rolling context band chart ─────────
def plot_band_overlay_with_context(results: pd.DataFrame, config: dict,
                                   n_bars: int = None, title: str = None,
                                   filename: str = "band_overlay_context.png") -> None:
    """
    Plot the standard execution VWAP sigma bands plus a secondary rolling
    context VWAP overlay.

    Parameters
    ----------
    results  : output DataFrame from run_backtest()
    config   : CONFIG dictionary
    n_bars   : number of recent bars to display (None = config value)
    title    : plot title override
    filename : output filename saved into artifacts/plots/
    """
    n_bars = n_bars or config.get('plot_last_n_bars')
    ...
    show_exec = config.get('show_execution_bands', True)
    show_context = config.get('show_context_bands', True)
    fill_zones = config.get('fill_band_zones', True)

    # ── Price panel ──
    if 'close' in plot_df.columns:
        ax1.plot(x, plot_df['close'], color='#212121', lw=1.0, label='Close', zorder=8)

    # Main execution VWAP / bands
    if show_exec and 'reference' in plot_df.columns:
        ax1.plot(
            x, plot_df['reference'],
            color='#1565C0', lw=1.6, label=config.get('reference_type', 'Execution VWAP'),
            zorder=7
        )

        exec_band_styles = [
            ('band_1p', 'band_1n', '#4CAF50', 0.15, 'Exec ±1σ'),
            ('band_2p', 'band_2n', '#FF9800', 0.10, 'Exec ±2σ'),
            ('band_3p', 'band_3n', '#F44336', 0.07, 'Exec ±3σ'),
        ]

        for upper, lower, color, alpha, label in exec_band_styles:
            if upper in plot_df.columns and lower in plot_df.columns:
                if fill_zones:
                    ax1.fill_between(
                        x, plot_df[upper], plot_df[lower],
                        color=color, alpha=alpha, zorder=1
                    )
                ax1.plot(x, plot_df[upper], color=color, lw=0.7, alpha=0.65, label=label, zorder=5)
                ax1.plot(x, plot_df[lower], color=color, lw=0.7, alpha=0.65, zorder=5)

    # Secondary rolling / bendy context VWAP
    if show_context and 'context_reference' in plot_df.columns:
        ax1.plot(
            x, plot_df['context_reference'],
            color='#7E57C2', lw=1.2, ls='--', alpha=0.95,
            label='Context VWAP', zorder=6
        )

        context_band_styles = [
            ('context_band_1p', 'context_band_1n', '#26A69A', 0.07, 'Context ±1σ'),
            ('context_band_2p', 'context_band_2n', '#AB47BC', 0.05, 'Context ±2σ'),
            ('context_band_3p', 'context_band_3n', '#5C6BC0', 0.035, 'Context ±3σ'),
        ]

        for upper, lower, color, alpha, label in context_band_styles:
            if upper in plot_df.columns and lower in plot_df.columns:
                if fill_zones:
                    ax1.fill_between(
                        x, plot_df[upper], plot_df[lower],
                ...
                ax1.plot(x, plot_df[upper], color=color, lw=0.8, alpha=0.70, ls='--', label=label, zorder=4)
                ax1.plot(x, plot_df[lower], color=color, lw=0.8, alpha=0.70, ls='--', zorder=4)

    # Zone background colouring
    if 'zone' in plot_df.columns:
        for i, row in plot_df.iterrows():
            zone_color = ZONE_COLORS.get(row['zone'], '#FFFFFF')
            ax1.axvspan(i - 0.5, i + 0.5, color=zone_color, alpha=0.04, zorder=0)

    ax1.set_ylabel('Price', fontsize=11)
    ax1.set_title(
        title or f"{config['instrument']} | Execution + Context VWAP Bands | Last {n_bars} bars",
        fontsize=13, fontweight='bold'
    )
    ax1.grid(True, alpha=0.2)
    ...
    if 'p_mr' in plot_df.columns:
        ax3.plot(x, plot_df['p_mr'], color='#4CAF50', lw=1.2, label='P(MR)')
    if 'p_cont' in plot_df.columns:
        ax3.plot(x, plot_df['p_cont'], color='#F44336', lw=1.2, label='P(Cont)')
    if 'p_neu' in plot_df.columns:
        ax3.plot(x, plot_df['p_neu'], color='#9E9E9E', lw=0.8, label='P(Neu)', alpha=0.6)
